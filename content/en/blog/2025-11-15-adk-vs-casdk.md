...
Many of you might have heard about agents. AI agents are an evolution of the initial language models that performed only discrete tasks (translate, answer a question, etc.).

They have a (limited) ability to reason and act. As one whitepaper describes, they usually incorporate the language model ("the brain"), some tools ("the hands"), and the orchestration layer ("the nervous system").

I had heard about agents, but I hadn't looked into them, and this course has been eye-opening.

To the point that I recognized some of the patterns (and gotchas) I had been following (and finding).
...
Once I discovered Google's Agent Development Kit (ADK), that workflow mapped naturally to a linear workflow agent with subagents. For Kids-Xowit, I could define a [Sequential Agent](https://google.github.io/adk-docs/agents/workflow-agents/sequential-agents/) that invokes:
...
This came as a bit of a surprise. I had thought CASDK was more advanced (certainly Claude Code, which is based on the SDK, seems very powerful). However, ADK felt significantly more powerful in its primitives.

Whether or not the underlying models are better is a completely different topic. It seems you can use ADK with, for example, [LiteLLM](https://www.litellm.ai/).
...
Instead, Claude Code (which is now an agent itself) detected it didn't have access to such a tool, so it proceeded to generate the Python code for a (narrow) tool that created the PDF, and then it invoked that tool.
